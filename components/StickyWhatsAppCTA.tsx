...
import { trackCtaClick, buildWhatsAppLink, type ServiceCode, type Intent } from '@/lib/tracking/cta'
...
const PAGE_TYPE_TO_SERVICE: Record<StickyWhatsAppCTAProps['pageType'], ServiceCode> = {
    sewa_kantor: 'office',
    virtual_office: 'vo',
    legal: 'legal',
}

const PAGE_TYPE_TO_INTENT: Record<StickyWhatsAppCTAProps['pageType'], Intent> = {
    sewa_kantor: 'consultation',
    virtual_office: 'consultation',
    legal: 'consultation',
}

export default function StickyWhatsAppCTA({ message, pageType, intent }: StickyWhatsAppCTAProps) {
    const [isVisible, setIsVisible] = useState(false)
    const phone = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '6281311778036'
    const service = PAGE_TYPE_TO_SERVICE[pageType]
    const resolvedIntent = intent || PAGE_TYPE_TO_INTENT[pageType]
    ...
    const handleClick = () => {
        ...
        // Backward-compat: keep Meta Pixel Contact event
